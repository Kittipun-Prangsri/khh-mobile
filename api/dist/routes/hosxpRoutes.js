...
import { getHosxpPatientByHnOrCid, getHosxpPatientList, getHosxpPatientMedicalHistory, getHosxpAppointments, getHosxpAppointmentsByHn, getHosxpAppointmentById, getHosxpMissedFollowUps, getHosxpNcdRegistry, getHosxpNcdRegistryStats, } from '../services/hosxpService.js';
const router = Router();
...
router.get('/patients/:hn/appointments', async (req, res) => {
    try {
        const { hn } = req.params;
        const limit = Number(req.query.limit) || 20;
        const data = await getHosxpAppointmentsByHn(hn, limit);
...
router.get('/registry', async (req, res) => {
    try {
        ...
        const controlStatus = req.query.controlStatus || 'all';
        const search = String(req.query.search || '');
        const page = Number(req.query.page) || 1;
        const limit = Number(req.query.limit) || 50;
