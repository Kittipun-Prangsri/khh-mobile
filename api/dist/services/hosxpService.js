 ...
 */
export async function getHosxpPatientByHnOrCid(query) {
    try {
        const pool = getHosxpPool();
        const cleanQuery = query.replace(/^HN-?/i, '');
        const [rows] = await pool.execute(`SELECT hn, pname, fname, lname, birthday, sex, cid, mobile_phone_number, hometel, informtel 
       FROM patient 
       WHERE hn = ? OR cid = ? 
       LIMIT 1`, [cleanQuery, cleanQuery]);
 ...
 */
export async function getHosxpPatientMedicalHistory(hn, limit = 10) {
    try {
        const pool = getHosxpPool();
        const cleanHn = hn.replace(/^HN-?/i, '');
        const [rows] = await pool.execute(`SELECT o.vn, o.vstdate, o.vsttime, s.bps, s.bpd, s.fbs, s.bw, s.height, s.bmi, s.pulse, v.pdx, v.dx0, v.dx1
       FROM ovst o
 ...
 */
export async function getHosxpAppointments(optionsOrLimit = 50) {
    try {
        const pool = getHosxpPool();
        let search = '';
        let startDate = '';
 ...
 */
export async function getHosxpAppointmentsByHn(hn, limit = 20) {
    return getHosxpAppointments({ hn, limit, clinic: 'all' });
}
/**
 * Query single appointment detail by `oapp_id` from HOSxP `oapp`
 */
export async function getHosxpAppointmentById(oappId) {
    try {
        const pool = getHosxpPool();
        const [rows] = await pool.execute(`SELECT o.oapp_id, 
              o.hn, 
              CONVERT(CONCAT(COALESCE(p.pname,''), COALESCE(p.fname,''), ' ', COALESCE(p.lname,'')) USING utf8mb4) AS patient_name, 
              COALESCE(p.mobile_phone_number, p.hometel, p.informtel) AS phone,
              p.cid,
 ...
 */
export async function getHosxpNcdRegistry(options = {}) {
    try {
        const pool = getHosxpPool();
    ...
    `;
        ...
        const patients = rows.map((r) => {
            ...
            if (r.clinic === '001')
                diseaseType = 'เบาหวาน (DM)';
            else if (r.clinic === '002')
                diseaseType = 'ความดันโลหิตสูง (HT)';
            else if (r.clinic === '030')
                diseaseType = 'โรคไตเรื้อรัง (CKD)';
        ...
        if (controlStatus === 'controlled') {
            filtered = patients.filter((p) => p.controlStatusCode === 'controlled');
        }
        else if (controlStatus === 'uncontrolled') {
            filtered = patients.filter((p) => p.controlStatusCode === 'uncontrolled');
        }
        else if (controlStatus === 'unknown') {
            filtered = patients.filter((p) => p.controlStatusCode === 'unknown');
        }
        return {
            patients: filtered,
            total: countRows[0]?.total || 0,
...
export async function getHosxpNcdRegistryStats() {
    try {
        const pool = getHosxpPool();
        const [dmCount] = await pool.execute("SELECT COUNT(DISTINCT hn) as total FROM clinicmember WHERE clinic = '001'");
        const [htCount] = await pool.execute("SELECT COUNT(DISTINCT hn) as total FROM clinicmember WHERE clinic = '002'");
        const [ckdCount] = await pool.execute("SELECT COUNT(DISTINCT hn) as total FROM clinicmember WHERE clinic = '030'");
        // Sample Registry calculation
        const registryData = await getHosxpNcdRegistry({ limit: 100 });
        const dmControlled = registryData.patients.filter((p) => p.clinicCode === '001' && p.isControlled).length;
        const dmTotalSample = registryData.patients.filter((p) => p.clinicCode === '001').length || 1;
        const htControlled = registryData.patients.filter((p) => p.clinicCode === '002' && p.isControlled).length;
        const htTotalSample = registryData.patients.filter((p) => p.clinicCode === '002').length || 1;
        const uncontrolledCount = registryData.patients.filter((p) => p.controlStatusCode === 'uncontrolled').length;
        return {
